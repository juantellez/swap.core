...
class PointSwap extends SwapInterface {
  ...
  constructor(options) {
    ...
    if (typeof options.fetchBalance !== 'function') {
      throw new Error('PointSwap: "fetchBalance" required')
    }
    if (typeof options.fetchUnspents !== 'function') {
      throw new Error('PointSwap: "fetchUnspents" required')
    }
    if (typeof options.broadcastTx !== 'function') {
      throw new Error('PointSwap: "broadcastTx" required')
    }
    if (typeof options.fetchTx !== 'function') {
      throw new Error('PointSwap: "fetchTx" required')
    }

    this._swapName      = constants.COINS.point
  ...
  _initSwap(app) {
    ...
    this.network = this.app.services.auth.accounts.point.network // TODO: templess solution, try to find better solution
     /*
     * this.network != this.app.services.auth.accounts.point.network
     *
     this.network = (
      this.app.isMainNet()
        ? this.app.env.coininfo.point.main
        : this.app.env.coininfo.point.test
    ).toBitcoinJS()
     */
  }
  ...
  _signTransaction(data) {
    ...
    const signature     = this.app.services.auth.accounts.point.sign(signatureHash).toScriptSignature(hashType)

    const scriptSig = this.app.env.bitcoin.script.scriptHash.input.encode(
      [
        signature,
        this.app.services.auth.accounts.point.getPublicKeyBuffer(),
  ...
  fundScript(data, handleTransactionHash, hashName) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const unspents      = await this.fetchUnspents(this.app.services.auth.accounts.point.getAddress())
        ...
        tx.addOutput(this.app.services.auth.accounts.point.getAddress(), skipValue)
        tx.inputs.forEach((input, index) => {
          tx.sign(index, this.app.services.auth.accounts.point)
        })
  ...
  async getWithdrawRawTransaction(data, isRefund, hashName) {
    ...
    tx.addOutput(this.app.services.auth.accounts.point.getAddress(), totalUnspent - feeValue)
